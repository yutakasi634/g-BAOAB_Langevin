...
function langevin_integration(coord_vec::Array{Float64, 2}, vel_vec::Array{Float64, 2},
    ...
    new_vel_vec =
        one_minus_gammah_over2 * (one_minus_gammah_over2 + (gamma * half_time_step)^2) .* vel_vec +
...
function BAOAB_langevin_integration(coord_vec::Array{Float64, 2}, vel_vec::Array{Float64, 2},
    ...
    temp_hat_vel_vec     =
        baoab_c_1 .* temp_vel_vec + baoab_c_3 .* sqrt_inv_mass_vec .* randn(rng, (3, particle_num)) # O step
